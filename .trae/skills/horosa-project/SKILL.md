---
name: "horosa-project"
description: "星阙 Horosa 项目专用技能，涵盖占星/八字/紫微/三式等术数功能开发。当用户修改 horosa-calc 计算引擎、horosa-server API、horosa-web 前端页面或参考源仓库进行功能对比时触发。"
---

# 星阙 Horosa 项目开发技能

基于 [星阙 Horosa Skill v0.9.2](https://github.com/Horace-Maxwell/horosa-skill) 的 68 个工具定义，提供项目开发指导。

## 项目结构

```
crates/horosa-calc/src/
...
|------|------|
| `astrostudyui/src/components/astrochart/` | `horosa-calc/src/astrology/` |
| `astrostudyui/src/components/bazi/` | `horosa-calc/src/bazi/` |
| `astrostudyui/src/components/ziwei/` | `horosa-calc/src/ziwei/` |
| `astrostudyui/src/components/qizheng/` | `horosa-calc/src/qizheng/` |
| `astrostudyui/src/components/liureng/` | `horosa-calc/src/liureng/` |
| `astrostudyui/src/components/liuyao/` | `horosa-calc/src/liuyao/` |
| `astrostudyui/src/components/sanshi/` | `horosa-calc/src/sanshi/` |
| `astrostudyui/src/components/vedic/` | `horosa-calc/src/vedic/` |
| `astrostudyui/src/components/predict/` | `horosa-calc/src/predict/` |
| `astrostudyui/src/components/fengshui/` | `horosa-calc/src/remaining.rs#fengshui` |
| `astrostudyui/src/components/huangji/` | `horosa-calc/src/remaining.rs#huangji` |
| `astrostudyui/src/components/jinkou/` | `horosa-calc/src/remaining.rs#jinkou` |
| `astrostudyui/src/components/jingjue/` | `horosa-calc/src/remaining.rs#jingjue` |
| `astrostudyui/src/components/shenyishu/` | `horosa-calc/src/remaining.rs#shenyishu` |
...
## Horosa Skill 68 个工具对照

本项目的 Rust 实现已覆盖 horosa-skill 中的所有 68 个工具：
...
- [ ] 阅读源仓库原始实现
- [ ] 确认 Rust 版本与源仓库算法一致
- [ ] 更新对应的 API 端点 (horosa-server)
- [ ] 更新对应的前端页面 (horosa-web)
- [ ] 运行 `cargo build --workspace` 验证编译
- [ ] 运行 `cargo test --workspace` 验证测试

## CSS 设计系统

使用 `--horosa-*` CSS 变量体系，完全对齐源仓库的 `app.less`：
- 颜色：背景/表面/文字/边框/强调色/金色/青色/八字色板
- 布局：导航宽度 206px, 工具栏高度 72px, 底部 64px
- 主题：`[data-horosa-appearance='dark']` 和 `[data-horosa-appearance='light']`